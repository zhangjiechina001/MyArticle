...
def calcMax(img):
    ...
    start_position=np.unravel_index(np.argmax(hist),hist.shape)
    start_position=start_position[0]
    ret_theta=(start_position/w)*np.pi
...
def unflood_imgPro(src,img1_info,img2_info):
    ...
    ret_img=unFlodImgPro(src,ret_info[1],ret_info[1]+(850/5925)*math.pi)
